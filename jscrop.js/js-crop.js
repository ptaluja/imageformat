      ...
      value: function (type, handler) {
        ...
        let removeHandler = function (value, index, source) {
          ...
          if (
            value.element === this &&
            value.type === type &&
            (eventHandler.__source__ || eventHandler) ===
              (handler.__source__ || handler)
          ) {
      ...
      if (!currentInstance) {
        ...
        let cropper = {
          ...
          resizeGrid: function (event) {
            ...
            switch (this.mouseDownElement) {
              ...
              case this.grid:
                ...
                if (
                  this.pageX >= this.imageLeft &&
                  this.pageX <= this.imageRight
                ) {
                  if (
                ...
                if (
                  this.pageY >= this.imageTop &&
                  this.pageY <= this.imageBottom
                ) {
                  if (
          ...
          stopResizingGrid: function (event) {
            ...
            document.detachEventHandler(
              "mouseup",
              this.stopResizingGrid.bind(this)
            );
            document.detachEventHandler(
              "mousemove",
              this.resizeGrid.bind(this)
            );
          ...
          initialiseGrid: function () {
            ...
            Object.entries(this.resizers).forEach(
              function ([key, value]) {
                value.attachEventHandler(
                  "mousedown",
                  this.startResizingGrid.bind(this)
                );
              }.bind(this)
            );
            this.grid.attachEventHandler(
              "mousedown",
              this.startResizingGrid.bind(this)
            );
            this.gridHolder.attachEventHandler(
              "transitionend",
              this.hideGrid.bind(this)
            );
        ...
        cropper.setOutputCanvas(
          options.outputCanvas || document.createElement("canvas")
        );
